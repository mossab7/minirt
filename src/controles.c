...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/14 22:01:17 by zbengued          #+#    #+#             */
/*   Updated: 2025/09/14 22:13:41 by zbengued         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (key == XK_Right)
		*trans = add_vec3(*trans, scale_vec3(program->scene->camera.right,
					TRANS_SPEED));
	if (key == XK_Left)
		*trans = add_vec3(*trans, scale_vec3(program->scene->camera.right,
					-TRANS_SPEED));
	if (key == XK_Up)
		*trans = add_vec3(*trans, scale_vec3(program->scene->camera.direction,
					TRANS_SPEED));
	if (key == XK_Down)
		*trans = add_vec3(*trans, scale_vec3(program->scene->camera.direction,
					-TRANS_SPEED));
	if (key == XK_space)
		*trans = add_vec3(*trans, scale_vec3(program->scene->camera.up,
					TRANS_SPEED));
	if (key == XK_Shift_L)
		*trans = add_vec3(*trans, scale_vec3(program->scene->camera.up,
					-TRANS_SPEED));
...
int	mouse_hook(int button, int x, int y, void *param)
{
	...
	ray = shoot_ray((*get_program())->scene, screen_pos);
	hit_info = find_closest_intersection((*get_program())->scene->objects,
			&ray);
	(void)program;
	if (hit_info.hit)
