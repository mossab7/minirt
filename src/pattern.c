...
/*   Updated: 2025/09/12 16:00:00 by gemini           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	apply_checkerboard_pattern(t_hit_info *hit_info, t_pattern *pattern)
{
	...
	{
		t_cone *cone = &hit_info->object->obj.cone;
		t_vec3 axis = normalize_vec3(cone->axis);
		t_vec3 vec_from_base = sub_vec3(hit_info->point, cone->center);
		u = atan2(hit_info->normal.x, hit_info->normal.z) / (2 * M_PI);
...
t_color	get_pattern_color(t_hit_info *hit_info, void *object)
{
	...
	else if (hit_info->object_type == CONE)
	{
		...
		return ((t_color){0,0,0}); // Should not happen

	if (pattern && (pattern->type & PATTERN_TEXTURE))
	{
		t_color texture_color = apply_texture(hit_info, pattern);
		double alpha = 0.5; // 50% mix factor

		return ((t_color){
			texture_color.r * (1.0 - alpha) + base_color.r * alpha,
			texture_color.g * (1.0 - alpha) + base_color.g * alpha,
